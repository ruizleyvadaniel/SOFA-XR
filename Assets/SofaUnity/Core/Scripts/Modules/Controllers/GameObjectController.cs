...
namespace SofaUnity
{
    public class GameObjectController : ObjectController
    {
        ...
        void FixedUpdate()
        {
            ...
            {
                if (Input.GetKey(KeyCode.A))//Keypad4
                    transform.eulerAngles = new Vector3(transform.eulerAngles.x, transform.eulerAngles.y + 0.5f, transform.eulerAngles.z);
                else if (Input.GetKey(KeyCode.D))//Keypad6
                    transform.eulerAngles = new Vector3(transform.eulerAngles.x, transform.eulerAngles.y - 0.5f, transform.eulerAngles.z);
                else if (Input.GetKey(KeyCode.W))//Keypad8
                    transform.eulerAngles = new Vector3(transform.eulerAngles.x + 0.5f, transform.eulerAngles.y, transform.eulerAngles.z);
                else if (Input.GetKey(KeyCode.S))//Keypad2
                    transform.eulerAngles = new Vector3(transform.eulerAngles.x - 0.5f, transform.eulerAngles.y, transform.eulerAngles.z);
                else if (Input.GetKey(KeyCode.F))//Keypad5 //MOVES Backward
                    transform.position = transform.position - transform.forward * factor;
            }
            else
            {
                if (Input.GetKey(KeyCode.W))//Keypad8 //MOVES UP
                    transform.position = transform.position + transform.up * factor;
                else if (Input.GetKey(KeyCode.S))//Keypad2 //MOVES DOWN
                    transform.position = transform.position - transform.up * factor;
                else if (Input.GetKey(KeyCode.A))//Keypad4 //MOVES LEFT
                    transform.position = transform.position - transform.right * factor;
                else if (Input.GetKey(KeyCode.D))//Keypad6) //MOVES RIGHT
                    transform.position = transform.position + transform.right * factor;
                else if (Input.GetKey(KeyCode.F))//Keypad5 //MOVES Forward
                    transform.position = transform.position + transform.forward * factor;
